#!/usr/bin/env python3
"""
plot_nwdaf_paper_drift.py
=========================
Visualises the NWDAF paper two-regime AR(1) drift simulation dataset
produced by generate_nwdaf_paper_drift.py.

Generates two chart files:
  1. nwdaf_paper_drift_overview.png  — 2-row overview (UL / DL) with all
     3 clients overlaid on the same axes, plus a regime-switch marker.
  2. nwdaf_paper_drift_clients.png   — 3-row × 2-col grid showing each
...
# ── File paths ────────────────────────────────────────────────────────────────
PARQUET_PATH = SCRIPT_DIR / "nwdaf_paper_drift_simulation.parquet"
OVERVIEW_PNG = SCRIPT_DIR / "nwdaf_paper_drift_overview.png"
CLIENTS_PNG = SCRIPT_DIR / "nwdaf_paper_drift_clients.png"
...
CLIENT_COLORS = [
    "#3B82F6",  # blue   — 10.10.0.1
    "#F97316",  # orange — 10.10.0.2
    "#10B981",  # emerald — 10.10.0.3
]
...
def plot_overview(df: pd.DataFrame, clients: list):
    ...
    fig.suptitle(
        "NWDAF Paper — Two-Regime AR(1) Drift Simulation (All Clients)",
...
def plot_per_client(df: pd.DataFrame, clients: list):
    ...
    fig.suptitle(
        "NWDAF Paper — Per-Client AR(1) Traffic  (UL / DL Volume)",
...
def main():
    print("=" * 70)
    print("NWDAF Paper Drift — Chart Generator")
    print("=" * 70)

    if not PARQUET_PATH.exists():
        print(f"[ERROR] Parquet not found: {PARQUET_PATH}")
        print("        Run generate_nwdaf_paper_drift.py first.")
        return
